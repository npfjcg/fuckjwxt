...
var user = "";
function createTableSetDiv(useraccount)
{
	user = useraccount;
	initFieldValue();
...
function LeftMoveShowField()
{
	...
	for (var i=0;i<ObjdShowField.options.length;i++) {         
		if (ObjdShowField.options[i].selected){
			var newoption = document.createElement("OPTION");
			ObjYShowField.options.add(newoption);
			newoption.innerText = ObjdShowField.options[i].text;
			newoption.value = ObjdShowField.options[i].value;				
		}
	}
	for (var j=ObjdShowField.options.length-1;j>=0;j--) {
		if (ObjdShowField.options[j].selected){
			ObjdShowField.options.remove(j);	
		}
	}
}
...
function RightMoveShowField()
{
	...
	for (var i=0;i<ObjYShowField.options.length;i++) {         
		if (ObjYShowField.options[i].selected){
			var newoption = document.createElement("OPTION");
			ObjdShowField.options.add(newoption);
			newoption.innerText = ObjYShowField.options[i].text;
			newoption.value = ObjYShowField.options[i].value;					
		}
	}
	for (var j=ObjYShowField.options.length-1;j>=0;j--) {
		if (ObjYShowField.options[j].selected){
			ObjYShowField.options.remove(j);	
		}
	}
}
...
function doChangeTableSet()
{
	...
	doCanCelTableSet();
	dwrMonitor.setFieldBaseBean(courrentUrl + user + "-f",document.getElementById("tableFields").value);
	dwrMonitor.setFieldBaseBean(courrentUrl + user + "-o",document.getElementById("otherFields").value);	
	window.Form1.submit();
}
var isCompleInitF=true;
function initFieldValue() {
	isCompleInitF=false;
	DWREngine.setAsync(false);
	dwrMonitor.getFieldValue(fieldValue);
	isCompleInitF=true;
}

function fieldValue(mapObj) {
	var valueF = mapObj[courrentUrl + user + "-f"];		
	if(valueF != null) {
		var divObj = document.getElementById("tableFields");
		if(divObj != null) {
			divObj.value = valueF;			
		}		
	}	
	var valueFo = mapObj[courrentUrl + user + "-o"];
	if(valueFo != null) {
		var divObj = document.getElementById("otherFields");
		if(divObj != null) {
			divObj.value = valueFo;						
		}
	}	
}
...
var courrentUrl1 = courrentUrl.split("?")[0];
var courrentUrl2 = courrentUrl.split("=")[1];
courrentUrl = courrentUrl1+courrentUrl2;
